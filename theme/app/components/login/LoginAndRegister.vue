<template>
  <div
    class="max-w-lg mx-auto my-16 text-center align-center flex flex-col justify-center border p-4 rounded-md"
  >
    <div class="flex flex-col my-8">
      ...
      <p v-else-if="formView === FormView.REGISTER" class="text-gray-500 mt-2">
        ...
        <a
          ...
          class="text-primary text-semibold cursor-pointer"
          >Sign in</a
        >.
      </p>
    </div>
    ...
    <LoginProviders
      class="mb-8"
      v-if="formView === FormView.LOGIN || formView === FormView.REGISTER"
    />
    ...
    <form @submit.prevent="handleFormSubmit(userInfo)">
      ...
      <input
        v-if="
          ...
          formView === FormView.FORGOT_PASSWORD
        "
        id="email"
      ...
      <div v-if="formView !== FormView.FORGOT_PASSWORD">
        ...
        <PasswordInput
          ...
          :autocomplete="
            formView === FormView.LOGIN ? 'current-password' : 'new-password'
          "
          :required="true"
        />
      </div>
...
const inputPlaceholder = computed(() => {
  return {
    ...
    username:
      formView.value === FormView.LOGIN ? "johndoe@email.com" : "johndoe",
    password: "********",
  };
});
</script>
...
form button {
  @apply rounded-lg font-bold  py-3 px-8 hover:bg-black hover:text-white;
}
</style>
