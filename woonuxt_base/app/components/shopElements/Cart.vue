...
<template>
  <div class="fixed top-0 bottom-0 right-0 z-50 flex flex-col w-11/12 max-w-lg overflow-x-hidden bg-gray-100 shadow-lg border-l">
    ...
    <ClientOnly>
      <template v-if="cart && !cart.isEmpty">
        <ul class="flex flex-col flex-1">
          <li v-for="item in cart.contents?.nodes" :key="item.key" class="border-b last:mb-4">
            <CartCard :item="item" />
          </li>
        </ul>
